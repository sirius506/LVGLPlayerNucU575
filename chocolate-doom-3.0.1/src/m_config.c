...
static void LoadDefaultCollection(default_collection_t *collection)
{
    ...
    f = OpenFATFile(collection->filename);
    ...
    while (!f_eof(f))
    {
        f_gets(linebuffer, sizeof(linebuffer), f);
    ...
    CloseFATFile(f);
#else
    /* We are going to load config file from QSPI flash. */
