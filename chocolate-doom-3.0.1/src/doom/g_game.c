...
#else
    save_stream = OpenFATFile(savename);
#endif
...
#else
    save_stream = CreateFATFile(temp_savegame_file);
#endif
...
#else
        save_stream = CreateFATFile(recovery_savegame_file);
#endif
        if (save_stream == NULL)
        {
            debug_printf("Failed to open either '%s' or '%s' to write savegame.",
...
#endif
    {
        debug_printf("Savegame buffer overrun");
