...
static struct {
  STM32_PALETTE palette[256];
  pixel_t VideoBufferSpace[SCREENWIDTH*SCREENHEIGHT];
} VideoImageBuffer;

static STM32_PALETTE *palette;

const uint8_t *GameImageData = (uint8_t *)&VideoImageBuffer;
#endif
...
void I_InitGraphics(void)
{
    ...
    doompal = W_CacheLumpName(DEH_String("PLAYPAL"), PU_CACHE);
    palette = VideoImageBuffer.palette;
    I_SetPalette(doompal);
#endif
...
#else
    I_VideoBuffer = VideoImageBuffer.VideoBufferSpace;
#endif
    V_RestoreBuffer();
