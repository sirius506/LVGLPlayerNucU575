...
static byte saveg_read8(void)
{
    ...
    if (f_read(save_stream, &result, 1, &nbr) != FR_OK)
    {
        if (!savegame_error)
        {
            debug_printf("saveg_read8: Unexpected end of file while "
                            "reading save game\n");
...
static void saveg_write8(byte value)
{
    ...
    if (f_write(save_stream, &value, 1, &nbw) != FR_OK)
    {
        if (!savegame_error)
        {
            debug_printf("saveg_write8: Error while writing save game\n");
