...
#ifdef STM32_DOOMPLAYER
static SECTION_AHBSRAM char linebuffer[100];
#endif

static void LoadDefaultCollection(default_collection_t *collection)
{
#ifdef STM32_DOOMPLAYER
    /* We are goingo to load config file from QSPI flash. */

    QSPI_DIRHEADER *dirhp = (QSPI_DIRHEADER *)QSPI_FLASH_ADDR;
    FS_DIRENT *direp;
    size_t i;
    char *cpos;
    const char *sp;
    ...
    // read the file in, overriding any set default
    ...
    f_len = 0;
    sp = collection->filename;
    if (*sp == '/') sp++;	/* Ignore root symbol */

    for (i = 0; i < dirhp->fs_fcount; i++)
    {
      if (direp->fsize == 0xFFFFFFFF)
        break;
      /* In QSPI direntry, max filename length is 16. */
      if (strncasecmp(direp->fname, sp, 16) == 0)
      {
        f_len = direp->fsize;
        cpos = (char *)(QSPI_FLASH_ADDR + direp->foffset);
        break;
      }
      direp++;
    }
    ...
    if (f_len == 0)
    {
      ...
      debug_printf("%s: can't load %s\n", __FUNCTION__, collection->filename);
      return;
    }
    ...
    while (f_len > 0)
    {
      ...
      epos = strchr(cpos, '\n');
      slen = epos - cpos + 1;
      memcpy(linebuffer, cpos, slen);
      linebuffer[slen] = 0;

      if (sscanf(linebuffer, "%79s %99[^\n]\n", defname, strparm) != 2)
      {
      ...
      {
        // Unknown variable? Unbound variable are also treated
        // as unknown.
...
#ifdef STM32_DOOMPLAYER
    M_SDMakeDirectory(prefdir);
    M_SDMakeDirectory(music_pack_path);
#else
